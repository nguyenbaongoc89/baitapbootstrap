...
<body>
    <div class="container">
        ...
        <table class="table tbl-1">
            ...
            <tbody>
                @if(count($resultDBView) > 0)
                    @foreach ($resultDBView as $key=>$row)
                        <tr>
                            <td scope="row">{{$key+1}}</td>
                            <td><a href="/product/show/{{$row->id}}">{{$row->name}}</a></td>
                            <td>{{$row->price}}</td>
                            <td>500</td>
                            ...
                            <td>
                            {{-- <img src="imgs/{{$row->img}}" alt="" style="width : 100px;height : 50px"> --}}
                            </td>
                            <td>
                                <button class="btn btn-success" type="button"><a href="/product/edit/{{$row->id}}">Edit</a> </button>
                            <button class="btn btn-danger btn-delete" idproduct="{{$row->id}}" type="button">Delete</button>
                            </td>
                        </tr>
                    @endforeach 
                @else
